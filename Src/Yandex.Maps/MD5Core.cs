...
internal sealed class MD5Core
{
  ...
  public static byte[] GetHash(string input) => MD5Core.GetHash(input, (Encoding) new UTF8Encoding());

  public static string GetHashString(byte[] input) => input != null ? BitConverter.ToString(MD5Core.GetHash(input)).Replace("-", "") : throw new ArgumentNullException(nameof (input), "Unable to calculate hash over null input data");
  ...
  public static string GetHashString(string input) => MD5Core.GetHashString(input, (Encoding) new UTF8Encoding());
  ...
  private static uint r1(uint a, uint b, uint c, uint d, uint x, int s, uint t) => b + MD5Core.LSR(a + (uint) ((int) b & (int) c | ((int) b ^ -1) & (int) d) + x + t, s);

  private static uint r2(uint a, uint b, uint c, uint d, uint x, int s, uint t) => b + MD5Core.LSR(a + (uint) ((int) b & (int) d | (int) c & ((int) d ^ -1)) + x + t, s);

  private static uint r3(uint a, uint b, uint c, uint d, uint x, int s, uint t) => b + MD5Core.LSR(a + (b ^ c ^ d) + x + t, s);

  private static uint r4(uint a, uint b, uint c, uint d, uint x, int s, uint t) => b + MD5Core.LSR(a + (c ^ (b | d ^ uint.MaxValue)) + x + t, s);
