    ...
    private void finish()
    {
      ...
      else
      {
        ...
        if (this._z.AvailableBytesIn < 8)
        {
          ...
          if (count != num)
            throw new ZlibException(string.Format("Protocol error. AvailableBytesIn={0}, expected 8", (object) (this._z.AvailableBytesIn + num)));
        }
        else
        ...
        if (num1 != int32_2)
          throw new ZlibException(string.Format("Bad size in GZIP stream. (actual({0})!=expected({1}))", (object) num1, (object) int32_2));
      }
    }
